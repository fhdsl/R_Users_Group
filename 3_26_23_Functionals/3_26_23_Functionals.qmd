...
-   A function to facilitate the looping mechanism: `map()`.
...
for(current_species in c("Adelie", "Chinstrap", "Gentoo")) {
  penguins_subset = filter(penguins, species == current_species)
  print(mean(penguins_subset$bill_length_mm, na.rm=TRUE))
}
```
...
for(col_name in names(penguins_numeric)) {
  col_vector = unlist(select(penguins_numeric, col_name))
  print(mean(col_vector, na.rm = TRUE))
}
```
